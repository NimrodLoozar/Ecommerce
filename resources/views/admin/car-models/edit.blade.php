    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('admin.car-models.update', $carModel) }}" method="POST">
                        @csrf
                        @method('PATCH')
                            ...
                            <select name="brand_id" id="brand_id"
                                ...
                                @foreach ($brands as $brand)
                                    <option value="{{ $brand->id }}" 
                                        {{ old('brand_id', $carModel->brand_id) == $brand->id ? 'selected' : '' }}>
                                        {{ $brand->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="description" id="description" rows="4"
                                ...
                                placeholder="Brief description of this car model (optional)">{{ old('description', $carModel->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                                ...
                                <div class="flex">
                                    ...
                                    <div>
                                        <p class="text-sm text-blue-700">
                                            This model has <strong>{{ $carModel->cars()->count() }}</strong> 
                                            {{ Str::plural('car', $carModel->cars()->count()) }} associated with it.
                                            Changes will not affect existing car listings.
                                        </p>
                                    </div>
                                </div>
                            </div>
